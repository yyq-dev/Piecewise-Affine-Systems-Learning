...
for t = max(n_a, n_b)+1:N_train
    ...
    phi_train(2:n_a+1, t) = y_train(t-1:-1:t-n_a); 
    phi_train(n_a+2:end, t) = u_train(t-1:-1:t-n_b);
end
...
rho = sdpvar(M, 1); % 不确定性集大小
eta = binvar(M, N_train, 'full'); % 二元变量 η_{it}
...
for i = 1:M
    for t = 1:N_train
        ...
        constraints = [constraints, rho(i) >= 0];
        % 修改后的不确定性集约束
        constraints = [constraints, ...
            varepsilon(t) <= rho(i) + M_w * (1 - eta(i, t))];
    end
    % 修改后的覆盖率约束
    constraints = [constraints, sum(eta(i, :)) >= p * N_train];
end
